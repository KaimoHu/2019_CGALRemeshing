#ifndef CGAL_ENRICHED_POLYHEDRON_H
#define CGAL_ENRICHED_POLYHEDRON_H
...
namespace CGAL {
  ...
  struct Vertex_normal {   //	(functor)
    ...
    void operator()(Vertex&	v) {
      ...
      Vertex::Halfedge_around_vertex_const_circulator	he = v.vertex_begin();
      Vertex::Halfedge_around_vertex_const_circulator	begin = he;
  ...
  class Enriched_polyhedron : public CGAL::Polyhedron_3<Kernel, Items> {
   public:
    ...
    typedef typename std::list<Halfedge_const_handle>::const_iterator
      Halfedge_const_iter;
    typedef typename std::list<Facet_handle> Facet_list;
    typedef typename std::list<Facet_handle>::iterator Facet_iter;
    typedef typename std::list<Facet_const_handle> Facet_const_list;
    typedef typename std::list<Facet_const_handle>::const_iterator
      Facet_const_iter;
    typedef typename std::list<Vertex_handle> Vertex_list;
    typedef typename std::list<Vertex_handle>::iterator Vertex_iter;
    typedef typename std::list<Vertex_const_handle> Vertex_const_list;
    ...
    Halfedge_handle get_local_maximal_error(
      ...
      assert(!facets.empty());
      std::set<Facet_handle>::iterator it = facets.begin();
      Facet_handle fh = *it;
    ...
    void generate_random_samples(bool use_stratified_sampling, int nb_samples,
      ...
      if (nb_samples <= 0) {
        ...
        // step 1г║ generate the unique disturbed border samples if necessary
    ...
    void trace_properties(std::string name) {
      std::cout << std::endl;
      transform(name.begin(), name.end(), name.begin(), std::toupper);
   ...
   private:
    ...
    void clear_local_in_links(std::set<Facet_handle> *in_link_facets) const {
      for (std::set<Facet_handle>::iterator it = in_link_facets->begin();
    ...
    void compute_triangle(const Point &p1, const Point &p2,
