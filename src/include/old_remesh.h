#ifndef OLD_REMESH_H_
#define OLD_REMESH_H_
...
class Old_remesh {
 public:
  ...
  typedef typename CPHalfedge<FT, Halfedge_handle> PHalfedge;       
  typedef typename CDPQueue_long<PHalfedge> DPQueue_halfedge_long;
  typedef typename CDPQueue_short<PHalfedge> DPQueue_halfedge_short;
  typedef typename CPVertex<FT, Vertex_handle> PVertex;
  typedef typename CDPQueue_long<PVertex> DPQueue_vertex_long;
  typedef typename CDPQueue_short<PVertex> DPQueue_vertex_short;
  typedef typename CPFacet<FT, Facet_handle> PFacet;
  typedef typename CDPQueue_long<PFacet> DPQueue_facet_long;
  typedef typename CDPQueue_short<PFacet> DPQueue_facet_short;
  ...
  Old_remesh(FT max_error_threshold,  // general parameters
    ...
    while (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
      ...
      while (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
        ...
        greedy_reduce_error(input_facet_tree, max_error_threshold,
          max_error, m_verbose_progress, false, &large_error_queue,
      ...
      if (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
        ...
        greedy_improve_angle(input_facet_tree, max_error_threshold,
          min_radian, m_verbose_progress, &large_error_queue,
    ...
    /*if (m_decrease_max_errors) {
      ...
      while (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
        ...
        while (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
          ...
          greedy_reduce_error(input_facet_tree, max_error_threshold,
            max_error, m_verbose_progress, false, NULL, NULL,
        ...
        if (m_pRemesh->size_of_vertices() < m_max_mesh_complexity &&
          ...
          greedy_improve_angle(input_facet_tree, max_error_threshold,
            min_radian, m_verbose_progress, NULL, NULL,
 ...
 private:
  ...
  void backup_local_in_links(const std::set<Facet_handle> &extended_facets,
    ...
    for (std::set<Facet_handle>::iterator it = extended_facets.begin();
  ...
  void remove_small_value_edges(const std::set<Facet_handle> &one_ring_facets,
    ...
    std::set<Facet_handle>::iterator it;
  ...
  void greedy_improve_angle(const Facet_tree &input_facet_tree,
      FT max_error_threshold, FT min_radian, bool verbose_progress,
      DPQueue_halfedge_long *large_error_queue,
      DPQueue_halfedge_short *small_radian_queue,
  ...
  void greedy_reduce_error(const Facet_tree &input_facet_tree,
    FT max_error_threshold, FT max_error, bool verbose_progress,
  ...
  Vertex_handle split_edge(const Facet_tree &input_facet_tree,
    ...
    if (m_inherit_element_types) {
      Halfedge_around_vertex_circulator vcirc = vh->vertex_begin();
      Halfedge_around_vertex_circulator vend = vcirc;
      std::map<Vertex_handle, bool>::iterator it;
      CGAL_For_all(vcirc, vend) {
  ...
  Vertex_handle construct_local_mesh(
    ...
    for (std::set<Facet_handle>::iterator it = differ_facets.begin();
