...
public:
  ...
  Point& get_point(vertex_descriptor vd) { return mesh_.point(vd); }

  const Point& get_point(vertex_descriptor vd) const { return mesh_.point(vd); }

  halfedge_descriptor get_opposite(halfedge_descriptor hd) const
  ...
  void calculate_max_squared_errors(std::set<face_descriptor> *faces) {
    ...
    for (auto it = faces->begin(); it != faces->end(); ++it) {
      ...
      // vertex in links
      Link_pointer_list &vertex_in_links = get_vertex_in_links(fd);
      ...
      for (int i = 0; i <= 2; ++i) {
        ...
        Link_list &edge_out_links = get_halfedge_out_links(hi);
  ...
  void sample_edge_links(const Face_tree &face_tree, 
    ...
    // precondition: the normal dihedral of hd is not -1.0
  ...
  void generate_local_links(const Face_tree &face_tree,
      bool reset_normal_dihedral, Link_iter_list *face_in_links,
      Link_iter_list *edge_in_links, Link_pointer_list *vertex_in_links,
      halfedge_descriptor hd, std::set<face_descriptor> *in_link_faces, 
      const NamedParameters &np) {
  ...
  void generate_local_links(const Face_tree &face_tree,
      bool reset_normal_dihedral, Link_iter_list *face_in_links,
      Link_iter_list *edge_in_links, Link_pointer_list *vertex_in_links,
      vertex_descriptor vd, std::set<face_descriptor> *in_link_faces,
      const NamedParameters &np) {
  ...
  void generate_local_in_links(const std::set<face_descriptor> &in_link_faces,
    Link_iter_list *face_in_links, Link_iter_list *edge_in_links,
    Link_pointer_list *vertex_in_links, bool use_local_aabb_tree) {
    Point_and_primitive_id pp;
  ...
  void restore_local_links(
      ...
      const Link &vertex_out, Link_iter_list *face_in_links,
      Link_iter_list *edge_in_links, Link_pointer_list *vertex_in_links,
      vertex_descriptor vd, std::set<face_descriptor> *in_link_faces,
      const NamedParameters &np) {
  ...
  void restore_local_in_links(const Point_list &face_in_end_points,
      Link_iter_list *face_in_links, const Point_list &edge_in_end_points,
  ...
  // queues (for eliminating degenerated faces)
  ...
  void optimize_vertex_position(const Face_tree &input_face_tree,
    Link_iter_list *face_in_links, Link_iter_list *edge_in_links,
    Link_pointer_list *vertex_in_links, vertex_descriptor vd,
    std::set<face_descriptor> *in_link_faces, const NamedParameters &np) {
  ...
  vertex_descriptor split_edge(const Face_tree &input_face_tree,
    ...
    if (np.inherit_element_types) {
      Halfedge_around_target_circulator hb(mesh_.halfedge(vd), mesh_), he(hb);
      std::map<vertex_descriptor, bool>::iterator it;
      do {
  ...
  bool is_collapsable(halfedge_descriptor hd) const {
    // precondition: hd is not a boundary halfedge (its opposite can be)
    if (is_border(hd)) {
      return false;
    }
  ...
  Point calculate_initial_point_for_collapse(halfedge_descriptor hd, 
                                       const NamedParameters &np) const {
  ...
  vertex_descriptor collapse_edge(const Face_tree &input_face_tree,
      ...
      DPQueue_halfedge_short *small_value_queue,
      Link_iter_list *face_in_links, Link_iter_list *edge_in_links,
      Link_pointer_list *vertex_in_links, halfedge_descriptor hd,
  ...
  bool relocate_vertex(const Face_tree &input_face_tree,
    ...
    if (!relocated) {
      ...
      restore_local_links(face_out_map, edge_out_map, vertex_out, 
          &face_in_links, &edge_in_links, &vertex_in_links, vd,
          &extended_faces, np);
      calculate_max_squared_errors(&extended_faces);
    }
  ...
  int relocate_vertices(const Face_tree &input_face_tree,
      ...
      bool reduce_complexity, DPQueue_halfedge_long *large_error_queue,
      DPQueue_halfedge_short *small_radian_queue,
      std::set<vertex_descriptor> *vertices, const NamedParameters &np) {
  ...
  inline vertex_descriptor get_opposite_vertex(halfedge_descriptor hd) {
    assert(!is_border(hd));
    if (is_border(hd)) {
  ...
  inline vertex_descriptor get_opposite_vertex(halfedge_descriptor hd) const {
    if (is_border(hd)) {
      std::cout << "Error definition of opposite vertex for border halfedge."
                << std::endl;
      return get_null_vertex();
  ...
  halfedge_descriptor get_longest_halfedge(face_descriptor fd) {
    halfedge_descriptor hd = mesh_.halfedge(fd);
  ...
  halfedge_descriptor get_minimal_radian(FT *minimal_radian) {
  ...
  halfedge_descriptor calculate_minimal_radian(FT *minimal_radian) const {
    *minimal_radian = CGAL_PI;
    halfedge_descriptor minimal_radian_hd;
    for (Mesh::Face_range::const_iterator fci = mesh_.faces().begin();
      fci != mesh_.faces().end(); ++fci) {
      FT radian = calculate_smallest_radian(*fci);
      if (radian < *minimal_radian) {
        *minimal_radian = radian;
        minimal_radian_hd = get_shortest_halfedge(*fci);
      }
    }
    return minimal_radian_hd;
  }

  halfedge_descriptor longest_side_propagation(halfedge_descriptor hd) {
  ...
  inline FT squared_length(halfedge_descriptor hd) const {
  ...
  void reset_face_tags(int value, const Face_list &faces) {
    for (auto it = faces.begin(); it != faces.end(); ++it) {
      face_descriptor fd = *it;
  ...
  Bbox calculate_bounding_box() const {
    ...
    Mesh::Vertex_range::iterator vi = mesh_.vertices().begin();
  ...
  FT inline calculate_feature_intensity(vertex_descriptor vd) const {
...
private:
  bool is_border(halfedge_descriptor hd) const { return mesh_.is_border(hd); }
  ...
  FT calculate_normal_dihedral(halfedge_descriptor hd) const {
    // precondition: hd is not on a border edge
  ...
  bool edge_flip_would_improve_radian(halfedge_descriptor hd) const {
    // precondition: hh or its opposite cannot be a boundary edge
    const Point &p = get_point(get_source_vertex(hd));
    const Point &q = get_point(get_target_vertex(hd));
    const Point &s = get_point(get_opposite_vertex(hd));
    const Point &t = get_point(get_opposite_vertex(get_opposite(hd)));
