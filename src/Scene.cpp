...
void Scene::compute_elements() {
  if (m_view_input && m_pInput != NULL) {
    ...
    minangle_remesh.compute_facets(m_draw_type, m_render_type, m_bbox, true, 
      m_pInput, &pos_input_faces, &pos_input_face_normals,
  ...
  if (m_view_remesh && m_pRemesh != NULL) {
    ...
    minangle_remesh.compute_facets(m_draw_type, m_render_type, m_bbox, false, 
      m_pRemesh, &pos_remesh_faces, &pos_remesh_face_normals,
      &pos_remesh_face_colors, &pos_remesh_boundaries, &pos_remesh_samples);
    // step 2: render the edges
    if (m_render_type == k_classifications) {
...
bool Scene::open(QString file_name) {
  ...
  minangle_remesh.calculate_feature_intensities("Input", m_pInput);
  update_bbox();
  m_pRemesh = new Polyhedron(*m_pInput);
  m_input_aabb_tree_constructed = false;
  m_links_initialized = false;
  minangle_remesh.initialize_private_data();
...
bool Scene::open_input(QString file_name) {
  ...
  minangle_remesh.clear_links(m_pInput, m_pRemesh);
  m_links_initialized = false;
  minangle_remesh.initialize_private_data();
...
bool Scene::open_remesh(QString file_name) {
  ...
  minangle_remesh.calculate_feature_intensities("Remesh", m_pRemesh);
  if (m_pInput == NULL) {
...
void Scene::eliminate_degenerations() {
  ...
  size_t nb_eliminations =
    minangle_remesh.eliminate_degenerated_facets(m_pInput);
...
void Scene::split_input_long_edges() {
  ...
  size_t nb_split = minangle_remesh.split_long_edges(m_pInput);
...
void Scene::isotropic_remeshing() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.isotropic_remeshing(m_input_facet_tree, m_bbox, m_pRemesh);
    changed();
  }
}
...
void Scene::initial_mesh_simplification() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.initial_mesh_simplification(m_input_facet_tree,
                                                m_bbox, m_pRemesh);
    reset_draw_render_types();
    changed();
  }
...
void Scene::split_local_longest_edge() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.split_local_longest_edge(m_input_facet_tree,
                                             m_bbox, m_pRemesh);
    reset_draw_render_types();
    changed();
  }
...
void Scene::increase_minimal_angle() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.increase_minimal_angle(m_input_facet_tree,
                                           m_bbox, m_pRemesh);
    reset_draw_render_types();
    changed();
  }
...
void Scene::maximize_minimal_angle() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.maximize_minimal_angle(m_input_facet_tree, 
                                           m_bbox, m_pRemesh);
    reset_draw_render_types();
    changed();
  }
...
void Scene::final_vertex_relocation() {
  if (m_pRemesh == NULL) {
    ...
    std::cout << std::endl;
    minangle_remesh.final_vertex_relocation(m_input_facet_tree, 
                                            m_bbox, m_pRemesh);
    reset_draw_render_types();
    changed();
  }
...
void Scene::generate() {
  if (m_pInput != NULL && m_pRemesh != NULL) {
    ...
    if (!m_input_aabb_tree_constructed) {
      minangle_remesh.build_facet_tree(*m_pInput, "Input", 
                                       &m_input_facet_tree);
      m_input_aabb_tree_constructed = true;
    }
    minangle_remesh.build_facet_tree(*m_pRemesh, "Remesh", 
                                     &m_remesh_facet_tree);
    minangle_remesh.generate_links(m_input_facet_tree, m_remesh_facet_tree,
                                   m_pInput, m_pRemesh);
  }
}
